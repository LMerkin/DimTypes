...
namespace DimTypes::Bits::CEMaths
{
  ...
  // "Abs":                                                                  //
  //-------------------------------------------------------------------------//
  // XXX: In CLang <= 18, "std::fabs" is not "constexpr" contrary to the C++23
...
#   endif
  ...
  // "FMod":                                                                 //
  //-------------------------------------------------------------------------//
  // Again, in CLang <= 18, "std::fmod" is not "constexpr", so our own implem-
  // entation is needed:
  //
  template<typename F>
...
# endif
  ...
  // compilers supporting C++ >= 23 (even in CLang <= 18 which is not entirely
  ...
  // XXX: Suppress bogus warnings generated by CLang <= 18 on the following
  ...
  //-------------------------------------------------------------------------//
  // XXX: In CLang <= 18,  this function  is STILL NOT "constexpr", because
...
#   else
    // XXX: In CLang <= 18, "std::frexp" is not "constexpr", so another method
...
# endif
  ...
  // Complex "Exp":                                                          //
  //-------------------------------------------------------------------------//
  // XXX: Not really "constexpr" in CLang <= 18:
  //
  template<typename T>
  ...
  //-------------------------------------------------------------------------//
  // Returns (cos(z), sin(z)). XXX: Not really "constexpr" in CLang <= 18:
  //
  template<typename T>
  ...
  // Complex "SqRt":                                                         //
  //-------------------------------------------------------------------------//
  // XXX: Also a placeholder as yet, NOT "constexpr" in CLang <= 18:
  //
  template<typename T>
